...
// isRegexPath checks if the path contains regex characters
...
func handleCreateOneError(ctx context.Context, err error, j SyncJob, dest SyncClient, sourcePath, destPath string) error {
	...
	backend.SetSyncStatus(ctx, j.SyncConfig.Namespace, j.SyncConfig.Name, backend.SyncStatusFailed)
	backend.WriteEvent(
		ctx,
...
func handleCreateOneSuccess(ctx context.Context, j SyncJob, dest SyncClient, sourcePath, destPath string) error {
	...
	backend.SetSyncStatus(ctx, j.SyncConfig.Namespace, j.SyncConfig.Name, backend.SyncStatusSuccess)
	backend.WriteEvent(
		ctx,
		j.SyncConfig.Namespace,
		j.SyncConfig.Name,
