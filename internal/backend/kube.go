...
func setSyncStatusKube(ctx context.Context, namespace string, name string, status SyncStatusString) error {
	...
	// Use Server-Side Apply to update the status
	return Reconciler.Patch(ctx, s, patch, client.FieldOwner("vault-secret-sync-controller"))
}

func SetSyncStatus(ctx context.Context, namespace string, name string, status SyncStatusString) error {
	if B == nil {
		return nil
	}
	switch B.Type() {
	case BackendTypeKubernetes:
		return setSyncStatusKube(ctx, namespace, name, status)
	default:
		return nil
	}
}
...
func AnnotationOperations(r *VaultSecretSyncReconciler, vaultSecretSync *v1alpha1.VaultSecretSync) error {
	...
	if vaultSecretSync.ObjectMeta.Annotations["force-sync"] != "" {
		...
		vaultSecretSync.Status.LastSyncTime = metav1.Now()
		...
		if err := ManualTrigger(context.Background(), *vaultSecretSync, op); err != nil {
			vaultSecretSync.Status.Status = string(SyncStatusFailed)
		...
		vaultSecretSync.Status.Status = string(SyncStatusSuccess)
		vaultSecretSync.Status.LastSyncTime = metav1.Now()

		// Update the VaultSecretSync object
		if err := r.Update(context.Background(), vaultSecretSync); err != nil {
			l.Errorf("failed to update object: %v", err)
			return err
		}
		if err := r.Status().Update(context.Background(), vaultSecretSync); err != nil {
			l.Errorf("failed to update status: %v", err)
			return err
		}
		r.Recorder.Event(vaultSecretSync, "Normal", "ManualTrigger", "Force-sync sync triggered")
		r.Recorder.Event(vaultSecretSync, "Normal", "Updated", "VaultSecretSync status updated to "+vaultSecretSync.Status.Status)
	}
...
func (r *VaultSecretSyncReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if vaultSecretSync.Status.SyncDestinations != len(vaultSecretSync.Spec.Dest) {
		vaultSecretSync.Status.SyncDestinations = len(vaultSecretSync.Spec.Dest)
		syncNow = true
	}
	...
	if objHash != vaultSecretSync.Status.Hash {
		vaultSecretSync.Status.Hash = objHash
		syncNow = true
	}
	...
	if syncNow {
		r.Recorder.Event(vaultSecretSync, "Normal", "Created", "VaultSecretSync created")
	...
	// Update the VaultSecretSync status and annotations
	l.Debugf("updating VaultSecretSync status: %+v", vaultSecretSync.Status)
	uo := &client.UpdateOptions{
		FieldManager: "vault-secret-sync-controller",
	}
	if err := r.Status().Update(context.Background(), vaultSecretSync); err != nil {
		return ctrl.Result{}, err
	}
	if err := r.Update(ctx, vaultSecretSync, uo); err != nil {
		return ctrl.Result{}, err
	}
